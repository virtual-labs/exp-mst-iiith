class mst_kruskal_demo_artefact {
  constructor() {
    ...
    this.addedEdges = 0;
    this.time;
    this.speed = 500; // Lower default interval for smoother animation
    this.paused = 0;
    this.graphNo = 0;
    this.nodes;
    this.edges;
...
function handlers() {
  ...
  document.getElementById("rege").onclick = function () {
    regen();
  };
  ...
  // Update speed immediately when slider is moved
  function updateSpeed() {
    var newSpeed = 6000 - document.getElementById("myRange").value * 60;
    mstp_demo_artefact.speed = newSpeed;
    if (mstp_demo_artefact.setSpeed) {
...
function clickededge2(edgeID) {
  if (edgeID) {
    ...
    clickedEdge.width = "3";
    graph.animateTraffic([{ edge: edgeID, trafficSize: 5, colorr: "brown" }]);
    edges.update(clickedEdge);
  }
}
...
function demo() {
  /* console.log(
    "demo() called, MST check:",
    mstp_demo_artefact.addedEdges,
    nodes.length
  ); */
  if (mstp_demo_artefact.addedEdges == nodes.length - 1) {
    // Collect MST edges as array of [from, to, weight]
    var mstEdges = [];
    var allEdges = edges.get();
    for (var i = 0; i < allEdges.length; i++) {
      var edge = allEdges[i];
      // Only include edges that are in MST (color brown)
      if (
        edge.color &&
        (edge.color.color === "brown" || edge.color === "brown")
      ) {
        mstEdges.push([edge.from, edge.to, edge.label || edge.weight]);
      }
    }
    var mstEdges = [];
    var allEdges = edges.get();
    var mstText = "";
    if (allEdges && Array.isArray(allEdges)) {
      for (var i = 0; i < allEdges.length; i++) {
        var edge = allEdges[i];
        // Only include edges that are in MST (color brown)
        if (
      ...
      if (mstEdges.length > 0) {
        ...
        mstText = '<span style="color:red">No MST edges found!</span>';
      }
    } else {
      mstText =
        '<span style="color:red">Error: Could not retrieve edge data!</span>';
    }
    document.getElementById("ins").innerHTML =
      "Finished adding n-1 edges so the edges in green form an MST.<br><b>MST Edges:</b> " +
      mstText;
    return;
    mstp_demo_artefact.addFlag = 0;
    mstp_demo_artefact.edgeNo++;
    if (mstp_demo_artefact.check_or_add == 0) {
      mstp_demo_artefact.check_or_add = 1;
    } else {
      mstp_demo_artefact.check_or_add = 0;
    }
    return;
  ...
  if (flag_1 == 0) {
    document.getElementById("ins").innerHTML =
      "Starting from node 'b' edges from the node are added to edges to be checked";
    var k;
    for (k in abc) {
      clickededge3(abc[k]);
    }
  ...
  if (mstp_demo_artefact.check_or_add == 0) {
    clickededge1(
      ans[mstp_demo_artefact.check_or_add][mstp_demo_artefact.edgeNo]
    );
    document.getElementById("ins").innerHTML =
      "Checking smallest edge in edges to be checked (edges in lime) that is " +
      ans[mstp_demo_artefact.check_or_add][mstp_demo_artefact.edgeNo];
  } else if (mstp_demo_artefact.check_or_add == 1) {
    if (
      ans[0][mstp_demo_artefact.edgeNo] == ans[1][mstp_demo_artefact.edgeNo]
    ) {
      clickededge2(
        ans[mstp_demo_artefact.check_or_add][mstp_demo_artefact.edgeNo]
      );
      ...
      document.getElementById("ins").innerHTML =
        "A node of edge " +
        ans[mstp_demo_artefact.check_or_add][mstp_demo_artefact.edgeNo] +
        " isn't in MST. So, it is added to MST";
      var k;
      mstp_demo_artefact.addFlag = 1;
      mstp_demo_artefact.edgeNo--;
    } else {
      document.getElementById("ins").innerHTML =
        "Nodes of edge " +
        ans[0][mstp_demo_artefact.edgeNo] +
        " are already in MST So, it is omitted from being added to MST";
...
function playPause() {
  if (mstp_demo_artefact.paused == 0) {
    mstp_demo_artefact.paused = 1;
    document.getElementById("pause").innerHTML = "Play";
    if (mstp_demo_artefact.setSpeed) {
      clearInterval(mstp_demo_artefact.setSpeed);
      mstp_demo_artefact.setSpeed = null;
    }
  } else if (mstp_demo_artefact.paused == 1) {
    mstp_demo_artefact.paused = 0;
    document.getElementById("pause").innerHTML = "Pause";
    if (!mstp_demo_artefact.setSpeed) {
      mstp_demo_artefact.setSpeed = setInterval(demo, mstp_demo_artefact.speed);
