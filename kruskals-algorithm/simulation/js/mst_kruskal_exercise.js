...
function regen() {
  ...
  updateObservation("");
  graph.setOptions({ interaction: { selectable: true } });
  selected = [];
...
for (i in ans[1]) {
  if (i != 0 && ans[1][i] != 0) mstk_pra_artefact.finalAnswer.push(ans[1][i]);
}

function find(final, val) {
  for (i in final) {
    if (final[i] == val) return true;
  }
  return false;
}
function updateDebugInfo(clear = false) {
  var debugBlock = document.getElementById("debug-block");
  if (clear) {
    document.getElementById("debug-selected").textContent = "[]";
    debugBlock.className = "comment-box";
    debugBlock.style.maxWidth = "350px";
    debugBlock.style.margin = "0.5vw auto 0.5vw auto";
    debugBlock.style.textAlign = "center";
...
function updateObservation(message) {
  document.getElementById("ins").textContent = message;
}

function submit() {
  if (selected.length != graphSet[mstk_pra_artefact.graphNo][0].length - 1) {
    updateObservation(
      "Spanning tree contains n-1 edges. So, selected MST is incorrect."
    );
    updateDebugInfo();
    return;
  }
  for (i in selected) {
    if (!find(mstk_pra_artefact.finalAnswer, selected[i])) {
      updateObservation("Incorrect graph selected.");
      updateDebugInfo();
      return;
    }
  }
  updateObservation("Selected spanning tree is correct.");
  updateDebugInfo();
}
...
graph.on("click", function (params) {
  ...
  if (!params["nodes"]["0"]) {
    if (!find(mstk_pra_artefact.toCheck, params["edges"]["0"])) {
      clickededge1(params["edges"]["0"]);
      mstk_pra_artefact.toCheck.push(params["edges"]["0"]);
      updateDebugInfo();
    } else {
      clickededge2(params["edges"]["0"]);
      if (!find(selected, params["edges"]["0"])) {
        selected.push(params["edges"]["0"]);
      }
      updateDebugInfo();
    }
  }
